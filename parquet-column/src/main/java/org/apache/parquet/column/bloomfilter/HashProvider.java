...
/**
 * Hash Provider for Bloom filter.
 */
public class HashProvider {
    private static final Log LOG = Log.getLog(HashProvider.class);
    ...
    public static int murmur3Signed(int seed, byte[] bytes) {
...
//        SHA512((bytes, m, k) -> HashProvider.hashCrypt(bytes, m, k, "SHA-512"));
        ...
        public HashMethod(HashFunction hashFunction) {
    ...
    public static class Murmur3 implements HashFunction {
        @Override
        public int[] hash(byte[] value, int m, int k) {
            return hashMurmur3(value, m, k);
        }
    }

    public static class Murmur3KirschMitzenmacher implements HashFunction {
        @Override
        public int[] hash(byte[] value, int m, int k) {
            return hashCassandra(value, m, k);
        }
    }

    public static class MD5 implements HashFunction {
        @Override
        public int[] hash(byte[] value, int m, int k) {
            return hashCrypt(value, m, k, "SHA-512");
        }
    }
    ...
    public static class SHA256 implements HashFunction {
        @Override
        public int[] hash(byte[] value, int m, int k) {
            return hashCrypt(value, m, k, "SHA-256");
        }
    }
